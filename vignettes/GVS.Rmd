...
The GVS takes as input a dataframe consiting of two columns: latitude and longitude (in that order). We provide an example dataset, `gvs_testfile`.
...
In addition to the coordinate resolution, the GVS offers metadata responses describing current code and database versions, and formatted acknowledgements and citations. For a quick overview, you can use the function `GVS_metadata`. If you use the GVS in a publication and need to cite it, proper attribution information is available with `GVS_citations`.
